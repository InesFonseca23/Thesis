####################################################################
#              SOUTHBOUND METRICS: SWITCH-TO-CONTROLLER            #
#                         PROACTIVE                                #
####################################################################
...
# Checks which hosts have a switch in common
def match_hosts(net):
    matched_hosts = []
        ...
        if len(connected_hosts) >= 2:
            for i in range(len(connected_hosts)):
                for j in range(i+1, len(connected_hosts)):
                    matched_hosts.append(((connected_hosts[i], connected_hosts[j]), switch)) # ((H1, H2), S1) -> H1 e H2 têm em comum o S1
    return matched_hosts

# Flow for each pair - reverse is required for confirmation to be returned
...
# NOT WORKING - https://test-odl-docs.readthedocs.io/en/latest/developer-guide/controller.html ctr-f "Create a new flow on the switch openflow:1 in table 2."
    ...
    if controller_name == 'onos': #WORKING
        for (source_host, destination_host), switch in matched_hosts:
            ...
            try:
                ...
                #else:
                #    print("Flows installed:", output_decoded) # Mesmo que o flow não seja instalado ele entra aqui
                #verify_flows = f"sudo ovs-ofctl dump-flows {switch} -O OpenFlow13" # Verifica se foi instalado com este!!
...
def ping_pair(pair):
    ...
    try:
        output = host2.cmd(f'ping -i 0.5 -w 60 {host1.IP()}') # ONOS only recognize odd-numbered hosts
        #print('output: ',output)
...
def initialize_ping(net):
    matched_hosts = match_hosts(net)
    with concurrent.futures.ThreadPoolExecutor() as executor: # hosts must be active at the same time
