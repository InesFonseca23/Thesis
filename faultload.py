...
def run_malformed_packets_generator_remotely(controller_name, controller_ip, total_packets, valid_percentage, mf_groups):
    ...
    try:
        ...
        # Malformed Packets Generation
        command = f"python3 ines/send.py {controller_name} {controller_ip} {total_packets} {valid_percentage} {mf_groups}"
        print(command)
...
def rest_failure_generator(controller_name, controller_ip, rest_port, num_faults, value_int, value_representation, value_string, fault_groups, maximum):
    print('Rest failure')
    api_spec = yaml_analyzer.yaml_analizer(controller_name) # Controllers Yaml File Analizer
    ...
    # Faults being generated in sequential blocks
    for _ in range(num_faults):
        while True:
            if controller_name == 'onos':
                path, path_data = random.choice(list(paths.items())) # Random endpoint to insert the failure
            ...
            if '{' in path and '}' in path:
                if method.upper() == 'GET': # if GET, insert into endpoint
                    modified_path = re.sub(r'\{[^}]*\}', fault_value.replace("\\", "\\\\"), path)
                if method.upper() == 'POST' or method.upper() == 'PUT': # ELSE, considers an correct endpoint and inserts the failure with PUT/POST
